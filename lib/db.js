...
db.exec(`
  ...
  -- Messagerie interne : une conversation par client, identifiée par son
  -- numéro de téléphone normalisé (pas de compte client à créer).
  ...
  CREATE TABLE IF NOT EXISTS messages (
    ...
    body TEXT NOT NULL,
    type TEXT NOT NULL DEFAULT 'text', -- 'text' ou 'audio'
    audioUrl TEXT DEFAULT '',
    createdAt TEXT NOT NULL DEFAULT (datetime('now'))
  );

  -- Abonnements Web Push : permet d'envoyer une notification même si la
  -- personne n'a pas l'onglet ouvert (côté client comme côté admin).
  CREATE TABLE IF NOT EXISTS push_subscriptions (
    id TEXT PRIMARY KEY,
    scope TEXT NOT NULL, -- 'admin' ou 'client'
    ownerId TEXT NOT NULL, -- id admin, ou id conversation pour un client
    endpoint TEXT UNIQUE NOT NULL,
    p256dh TEXT NOT NULL,
    auth TEXT NOT NULL,
    createdAt TEXT NOT NULL DEFAULT (datetime('now'))
  );
  -- Réglages généraux modifiables depuis l'espace pro (ex. coordonnées de contact).
...
if (!messageColumns.includes("audioUrl")) {
  db.exec("ALTER TABLE messages ADD COLUMN audioUrl TEXT DEFAULT ''");
}
