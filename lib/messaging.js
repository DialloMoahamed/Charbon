...
function addMessage(conversationId, sender, senderName, body) {
  const message = {
  ...
  db.prepare(
    `INSERT INTO messages (id, conversationId, sender, senderName, body) VALUES (@id, @conversationId, @sender, @senderName, @body)`
  ).run(message);
